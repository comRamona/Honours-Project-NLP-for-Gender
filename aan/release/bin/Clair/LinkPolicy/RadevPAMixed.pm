package Clair::LinkPolicy::RadevPAMixed;
...
Generates a corpus using the RadevPAMixed  model.
...
sub create_corpus {
  ...
  foreach $tgt_file (@tf_files) {
    foreach $src_file (@tf_files) {
      ...
      # Decide whether to use R-based or preferential attatchment links
      if (random_uniform() < $rP) {
        # Use R-based
        ...
        # Use preferential attatchment links
        # make sure we have an in-degree for the target node (fix this)
        unless (exists $indegree{$tgt_file}) {
          $indegree{$tgt_file} = 1;
        }
        # Compute probability of a link.
        if (random_uniform() < $indegree{$tgt_file}/$timestep) {
            # Grab a high TF*W term
            $term = (sort {$tfw_models{$tgt_file}->{$b} <=>
                           $tfw_models{$tgt_file}->{$a}} 
                          keys %{$tfw_models{$tgt_file}})[0];
            print LINKS "$src_file $tgt_file $term\n";
            $indegree{$tgt_file}++;
        }
      } # PA or Radev?
...
Takes a TF*W value "x", a threshold steepness "s", and a threshold
 center "c"
